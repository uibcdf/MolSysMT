...
def view_with_NGLView(item, atom_indices='all', structure_indices='all',
    ...
    from molsysmt.api_forms.api_molsysmt_MolSys import to_NGLView as molsysmt_MolSys_to_NGLView
...
def get_atom_id_from_atom(item, indices='all', structure_indices='all'):
    ...
    from molsysmt.api_forms.api_openmm_Topology import get_atom_id_from_atom as _get
...
def get_atom_name_from_atom(item, indices='all', structure_indices='all'):
    ...
    from molsysmt.api_forms.api_openmm_Topology import get_atom_name_from_atom as _get
    tmp_item = to_openmm_Topology(item)
    return _get(tmp_item, indices=indices, structure_indices=structure_indices)

def get_atom_type_from_atom(item, indices='all', structure_indices='all'):

    from molsysmt.api_forms.api_openmm_Topology import get_atom_type_from_atom as _get
    tmp_item = to_openmm_Topology(item)
    return _get(tmp_item, indices=indices, structure_indices=structure_indices)

def get_group_index_from_atom (item, indices='all', structure_indices='all'):

    from molsysmt.api_forms.api_openmm_Topology import get_group_index_from_atom as _get
    tmp_item = to_openmm_Topology(item)
    return _get(tmp_item, indices=indices, structure_indices=structure_indices)

def get_component_index_from_atom (item, indices='all', structure_indices='all'):

    from molsysmt.api_forms.api_openmm_Topology import get_component_index_from_atom as _get
    tmp_item = to_openmm_Topology(item)
    return _get(tmp_item, indices=indices, structure_indices=structure_indices)

def get_chain_index_from_atom (item, indices='all', structure_indices='all'):

    from molsysmt.api_forms.api_openmm_Topology import get_chain_index_from_atom as _get
    tmp_item = to_openmm_Topology(item)
    return _get(tmp_item, indices=indices, structure_indices=structure_indices)

def get_molecule_index_from_atom (item, indices='all', structure_indices='all'):

    from molsysmt.api_forms.api_openmm_Topology import get_molecule_index_from_atom as _get
    tmp_item = to_openmm_Topology(item)
    return _get(tmp_item, indices=indices, structure_indices=structure_indices)

def get_entity_index_from_atom (item, indices='all', structure_indices='all'):

    from molsysmt.api_forms.api_openmm_Topology import get_entity_index_from_atom as _get
    tmp_item = to_openmm_Topology(item)
    return _get(tmp_item, indices=indices, structure_indices=structure_indices)
...
def get_n_inner_bonds_from_atom (item, indices='all', structure_indices='all'):
    ...
    from molsysmt.api_forms.api_openmm_Topology import get_n_inner_bonds_from_atom as _get
    tmp_item = to_openmm_Topology(item)
    return _get(tmp_item, indices=indices, structure_indices=structure_indices)

def get_coordinates_from_atom(item, indices='all', structure_indices='all'):

    from molsysmt.api_forms.api_openmm_Context import get_coordinates_from_atom as _get
...
def get_group_id_from_group(item, indices='all', structure_indices='all'):
    ...
    from molsysmt.api_forms.api_openmm_Topology import get_group_id_from_group as _get
    tmp_item = to_openmm_Topology(item)
    return _get(tmp_item, indices=indices, structure_indices=structure_indices)

def get_group_name_from_group(item, indices='all', structure_indices='all'):

    from molsysmt.api_forms.api_openmm_Topology import get_group_name_from_group as _get
    tmp_item = to_openmm_Topology(item)
    return _get(tmp_item, indices=indices, structure_indices=structure_indices)

def get_group_type_from_group(item, indices='all', structure_indices='all'):

    from molsysmt.api_forms.api_openmm_Topology import get_group_type_from_group as _get
    tmp_item = to_openmm_Topology(item)
    return _get(tmp_item, indices=indices, structure_indices=structure_indices)
...
def get_component_name_from_component (item, indices='all', structure_indices='all'):
    ...
    from molsysmt.api_forms.api_openmm_Topology import get_component_name_from_component as _get
    tmp_item = to_openmm_Topology(item)
    return _get(tmp_item, indices=indices, structure_indices=structure_indices)
...
def get_component_type_from_component (item, indices='all', structure_indices='all'):
    ...
    from molsysmt.api_forms.api_openmm_Topology import get_component_type_from_component as _get
    tmp_item = to_openmm_Topology(item)
    return _get(tmp_item, indices=indices, structure_indices=structure_indices)
...
def get_molecule_id_from_molecule (item, indices='all', structure_indices='all'):
    ...
    from molsysmt.api_forms.api_openmm_Topology import get_molecule_id_from_molecule as _get
    tmp_item = to_openmm_Topology(item)
    return _get(tmp_item, indices=indices, structure_indices=structure_indices)

def get_molecule_name_from_molecule (item, indices='all', structure_indices='all'):

    from molsysmt.api_forms.api_openmm_Topology import get_molecule_name_from_molecule as _get
    tmp_item = to_openmm_Topology(item)
    return _get(tmp_item, indices=indices, structure_indices=structure_indices)
...
def get_chain_id_from_chain (item, indices='all', structure_indices='all'):
    ...
    from molsysmt.api_forms.api_openmm_Topology import get_chain_id_from_chain as _get
    tmp_item = to_openmm_Topology(item)
    return _get(tmp_item, indices=indices, structure_indices=structure_indices)

def get_chain_type_from_chain (item, indices='all', structure_indices='all'):

    from molsysmt.api_forms.api_openmm_Topology import get_chain_type_from_chain as _get
    tmp_item = to_openmm_Topology(item)
    return _get(tmp_item, indices=indices, structure_indices=structure_indices)
...
def get_entity_id_from_entity (item, indices='all', structure_indices='all'):
    ...
    from molsysmt.api_forms.api_openmm_Topology import get_entity_id_from_entity as _get
    tmp_item = to_openmm_Topology(item)
    return _get(tmp_item, indices=indices, structure_indices=structure_indices)

def get_entity_name_from_entity (item, indices='all', structure_indices='all'):

    from molsysmt.api_forms.api_openmm_Topology import get_entity_name_from_entity as _get
    tmp_item = to_openmm_Topology(item)
    return _get(tmp_item, indices=indices, structure_indices=structure_indices)

def get_entity_type_from_entity (item, indices='all', structure_indices='all'):

    from molsysmt.api_forms.api_openmm_Topology import get_entity_type_from_entity as _get
    tmp_item = to_openmm_Topology(item)
    return _get(tmp_item, indices=indices, structure_indices=structure_indices)
...
def get_n_atoms_from_system(item, indices='all', structure_indices='all'):
    ...
    from molsysmt.api_forms.api_openmm_Topology import get_n_atoms_from_system as _get
    tmp_item = to_openmm_Topology(item)
    return _get(tmp_item, indices=indices, structure_indices=structure_indices)

def get_n_groups_from_system(item, indices='all', structure_indices='all'):

    from molsysmt.api_forms.api_openmm_Topology import get_n_groups_from_system as _get
    tmp_item = to_openmm_Topology(item)
    return _get(tmp_item, indices=indices, structure_indices=structure_indices)

def get_n_components_from_system(item, indices='all', structure_indices='all'):

    from molsysmt.api_forms.api_openmm_Topology import get_n_components_from_system as _get
    tmp_item = to_openmm_Topology(item)
    return _get(tmp_item, indices=indices, structure_indices=structure_indices)

def get_n_chains_from_system(item, indices='all', structure_indices='all'):

    from molsysmt.api_forms.api_openmm_Topology import get_n_chains_from_system as _get
    tmp_item = to_openmm_Topology(item)
    return _get(tmp_item, indices=indices, structure_indices=structure_indices)

def get_n_molecules_from_system(item, indices='all', structure_indices='all'):

    from molsysmt.api_forms.api_openmm_Topology import get_n_molecules_from_system as _get
    tmp_item = to_openmm_Topology(item)
    return _get(tmp_item, indices=indices, structure_indices=structure_indices)
...
def get_n_bonds_from_system(item, indices='all', structure_indices='all'):
    ...
    from molsysmt.api_forms.api_openmm_Topology import get_n_bonds_from_system as _get
    tmp_item = to_openmm_Topology(item)
    return _get(tmp_item, indices=indices, structure_indices=structure_indices)

def get_coordinates_from_system(item, indices='all', structure_indices='all'):

    from molsysmt.api_forms.api_openmm_Context import get_coordinates_from_system as _get
    tmp_item = to_openmm_Context(item)
    return _get(tmp_item, indices=indices, structure_indices=structure_indices)

def get_box_from_system(item, indices='all', structure_indices='all'):

    from molsysmt.api_forms.api_openmm_Context import get_box_from_system as _get
    tmp_item = to_openmm_Context(item)
    return _get(tmp_item, indices=indices, structure_indices=structure_indices)

def get_box_shape_from_system(item, indices='all', structure_indices='all'):

    from molsysmt.api_forms.api_openmm_Context import get_box_shape_from_system as _get
    tmp_item = to_openmm_Context(item)
    return _get(tmp_item, indices=indices, structure_indices=structure_indices)

def get_box_lengths_from_system(item, indices='all', structure_indices='all'):

    from molsysmt.api_forms.api_openmm_Context import get_box_lengths_from_system as _get
    tmp_item = to_openmm_Context(item)
    return _get(tmp_item, indices=indices, structure_indices=structure_indices)

def get_box_angles_from_system(item, indices='all', structure_indices='all'):

    from molsysmt.api_forms.api_openmm_Context import get_box_angles_from_system as _get
    tmp_item = to_openmm_Context(item)
    return _get(tmp_item, indices=indices, structure_indices=structure_indices)

def get_time_from_system(item, indices='all', structure_indices='all'):

    from molsysmt.api_forms.api_openmm_Context import get_time_from_system as _get
    tmp_item = to_openmm_Context(item)
    return _get(tmp_item, indices=indices, structure_indices=structure_indices)
...
def get_bonded_atoms_from_system(item, indices='all', structure_indices='all'):
    ...
    from molsysmt.api_forms.api_openmm_Topology import get_bonded_atoms_from_system as _get
    tmp_item = to_openmm_Topology(item)
    return _get(tmp_item, indices=indices, structure_indices=structure_indices)
...
def get_bond_type_from_bond(item, indices='all', structure_indices='all'):
    ...
    from molsysmt.api_forms.api_openmm_Topology import get_bond_type_from_bond as _get
    tmp_item = to_openmm_Topology(item)
    return _get(tmp_item, indices=indices, structure_indices=structure_indices)

def get_atom_index_from_bond(item, indices='all', structure_indices='all'):

    from molsysmt.api_forms.api_openmm_Topology import get_atom_index_from_bond as _get
    tmp_item = to_openmm_Topology(item)
    return _get(tmp_item, indices=indices, structure_indices=structure_indices)
...
def set_box_to_system(item, indices='all', structure_indices='all', value=None):
    ...
    from molsysmt.api_forms.api_openmm_Context import set_box_to_system as _set
    return _set(item.context, indices=indices, structure_indices=structure_indices, value=value)

def set_coordinates_to_system(item, indices='all', structure_indices='all', value=None):

    from molsysmt.api_forms.api_openmm_Context import set_coordinates_to_system as _set
    return _set(item.context, indices=indices, structure_indices=structure_indices, value=value)
