...
def _aux_getter_big_attribute_from_small(item, attribute, from_target, indices):
    ...
    auxtarget_index_from_target = get(item, indices=indices, target=from_target, **dict_auxtarget_index)
    auxtarget_indices = np.unique(auxtarget_index_from_target)
    attribute_from_auxtarget = get(item, indices=auxtarget_indices, target=auxtarget, **dict_attribute)
...
def _aux_getter_big_index_from_small(item, attribute, from_target, indices):
    ...
    atom_index_from_target = get(item, indices=indices, target=from_target, atom_index=True)
    first_atom_index_from_target = np.array([ii[0] for ii in atom_index_from_target])
    output = get(item, indices=first_atom_index_from_target, target='atom', **dict_attribute)
    return output
...
def _aux2_getter_big_index_from_small(item, attribute, from_target, indices):
    ...
    atom_index_from_target = get(item, indices=indices, target=from_target, atom_index=True)
    attribute_from_atom = get(item, indices='all', target='atom', **dict_bbb)
...
def _aux2_getter_small_index_from_big(item, attribute, from_target, indices):
    ...
    indices_aux = get(item, indices=indices, target=from_target, **dict_index_from_target)

    attribute_from_atom = get(item, indices='all', target='atom', **dict_attribute)
    target_index_from_atom = get(item, indices='all', target='atom', **dict_index_from_target)
...
def get_atom_index_from_atom(item, indices='all', structure_indices='all', check=True):

    return _aux_getter_index(item, 'atom', indices)

#def get_atom_id_from_atom (item, indices='all', structure_indices='all', check=True):
#
#    return _aux_getter_attribute(item, 'id', 'atom', indices)
#
#def get_atom_name_from_atom (item, indices='all', structure_indices='all', check=True):
#
#    return _aux_getter_attribute(item, 'name', 'atom', indices)
#
#def get_atom_type_from_atom (item, indices='all', structure_indices='all', check=True):
#
#    return _aux_getter_attribute(item, 'type', 'atom', indices)

def get_group_id_from_atom (item, indices='all', structure_indices='all', check=True):

    return _aux_getter_big_attribute_from_small(item, 'group_id', 'atom', indices)

def get_group_name_from_atom (item, indices='all', structure_indices='all', check=True):

    output = _aux_getter_big_attribute_from_small(item, 'group_name', 'atom', indices)
    return output.astype(object)

def get_group_type_from_atom (item, indices='all', structure_indices='all', check=True):

    output = _aux_getter_big_attribute_from_small(item, 'group_type', 'atom', indices)
    return output.astype(object)

#def get_component_index_from_atom (item, indices='all', structure_indices='all', check=True):
#    raise NotImplementedError

def get_component_id_from_atom (item, indices='all', structure_indices='all', check=True):

    return _aux_getter_big_attribute_from_small(item, 'component_id', 'atom', indices)

def get_component_name_from_atom (item, indices='all', structure_indices='all', check=True):

    return _aux_getter_big_attribute_from_small(item, 'component_name', 'atom', indices)

def get_component_type_from_atom (item, indices='all', structure_indices='all', check=True):

    return _aux_getter_big_attribute_from_small(item, 'component_type', 'atom', indices)

#def get_chain_index_from_atom (item, indices='all', structure_indices='all', check=True):
#
#    raise NotImplementedError

def get_chain_id_from_atom (item, indices='all', structure_indices='all', check=True):

    return _aux_getter_big_attribute_from_small(item, 'chain_id', 'atom', indices)

def get_chain_name_from_atom (item, indices='all', structure_indices='all', check=True):

    return _aux_getter_big_attribute_from_small(item, 'chain_name', 'atom', indices)

def get_chain_type_from_atom (item, indices='all', structure_indices='all', check=True):

    return _aux_getter_big_attribute_from_small(item, 'chain_type', 'atom', indices)

#def get_molecule_index_from_atom (item, indices='all', structure_indices='all', check=True):
#
#    raise NotImplementedError

def get_molecule_id_from_atom (item, indices='all', structure_indices='all', check=True):

    return _aux_getter_big_attribute_from_small(item, 'molecule_id', 'atom', indices)

def get_molecule_name_from_atom (item, indices='all', structure_indices='all', check=True):

    return _aux_getter_big_attribute_from_small(item, 'molecule_name', 'atom', indices)

def get_molecule_type_from_atom (item, indices='all', structure_indices='all', check=True):

    return _aux_getter_big_attribute_from_small(item, 'molecule_type', 'atom', indices)

#def get_entity_index_from_atom (item, indices='all', structure_indices='all', check=True):
#
#    raise NotImplementedError

def get_entity_id_from_atom (item, indices='all', structure_indices='all', check=True):

    return _aux_getter_big_attribute_from_small(item, 'entity_id', 'atom', indices)

def get_entity_name_from_atom (item, indices='all', structure_indices='all', check=True):

    return _aux_getter_big_attribute_from_small(item, 'entity_name', 'atom', indices)

def get_entity_type_from_atom (item, indices='all', structure_indices='all', check=True):

    return _aux_getter_big_attribute_from_small(item, 'entity_type', 'atom', indices)

def get_n_atoms_from_atom (item, indices='all', structure_indices='all', check=True):

    return _aux_n(item, 'atom', indices)

def get_n_groups_from_atom (item, indices='all', structure_indices='all', check=True):

    return _aux_n_big_from_small(item, 'n_groups', 'atom', indices)

def get_n_components_from_atom (item, indices='all', structure_indices='all', check=True):

    return _aux_n_big_from_small(item, 'n_components', 'atom', indices)

def get_n_molecules_from_atom (item, indices='all', structure_indices='all', check=True):

    return _aux_n_big_from_small(item, 'n_molecules', 'atom', indices)

def get_n_chains_from_atom (item, indices='all', structure_indices='all', check=True):

    return _aux_n_big_from_small(item, 'n_chains', 'atom', indices)

def get_n_entities_from_atom (item, indices='all', structure_indices='all', check=True):

    return _aux_n_big_from_small(item, 'n_entities', 'atom', indices)

def get_bonded_atoms_from_atom (item, indices='all', structure_indices='all', check=True):
...
def get_bond_index_from_atom (item, indices='all', structure_indices='all', check=True):
...
def get_n_bonds_from_atom (item, indices='all', structure_indices='all', check=True):
...
def get_inner_bond_index_from_atom (item, indices='all', structure_indices='all', check=True):
...
#def get_inner_bonded_atoms_from_atom (item, indices='all', structure_indices='all', check=True):
#
#    raise NotImplementedError
...
#def get_frame_from_atom(item, indices='all', structure_indices='all', check=True):
#
#    from molsysmt.basic import get
#
...
def get_n_structures_from_atom(item, indices='all', structure_indices='all', check=True):

    from molsysmt.basic import get

    return get(item, indices='all', structure_indices=structure_indices, target='system', n_structures=True)

## group

def get_atom_index_from_group(item, indices='all', structure_indices='all', check=True):

    return _aux_getter_small_index_from_big(item, 'atom_index', 'group', indices)

def get_atom_id_from_group(item, indices='all', structure_indices='all', check=True):

    return _aux_getter_small_attribute_from_big(item, 'atom_id', 'group', indices)

def get_atom_name_from_group(item, indices='all', structure_indices='all', check=True):

    return _aux_getter_small_attribute_from_big(item, 'atom_name', 'group', indices)

def get_atom_type_from_group(item, indices='all', structure_indices='all', check=True):

    return _aux_getter_small_attribute_from_big(item, 'atom_type', 'group', indices)

def get_group_index_from_group(item, indices='all', structure_indices='all', check=True):

    return _aux_getter_index(item, 'group', indices)

#def get_group_id_from_group (item, indices='all', structure_indices='all', check=True):
#
#    return _aux_getter_attribute(item, 'id', 'group', indices)
#
#def get_group_name_from_group (item, indices='all', structure_indices='all', check=True):
#
#    return _aux_getter_attribute(item, 'name', 'group', indices)
#
#def get_group_type_from_group (item, indices='all', structure_indices='all', check=True):
#
#    return _aux_getter_attribute(item, 'type', 'group', indices)

def get_component_index_from_group (item, indices='all', structure_indices='all', check=True):

    return _aux_getter_big_index_from_small(item, 'component_index', 'group', indices)

def get_component_id_from_group (item, indices='all', structure_indices='all', check=True):

    return _aux_getter_big_attribute_from_small(item, 'component_id', 'group', indices)

def get_component_name_from_group (item, indices='all', structure_indices='all', check=True):

    return _aux_getter_big_attribute_from_small(item, 'component_name', 'group', indices)

def get_component_type_from_group (item, indices='all', structure_indices='all', check=True):

    return _aux_getter_big_attribute_from_small(item, 'component_type', 'group', indices)

def get_chain_index_from_group (item, indices='all', structure_indices='all', check=True):

    return _aux_getter_big_index_from_small(item, 'chain_index', 'group', indices)

def get_chain_id_from_group (item, indices='all', structure_indices='all', check=True):

    return _aux_getter_big_attribute_from_small(item, 'chain_id', 'group', indices)

def get_chain_name_from_group (item, indices='all', structure_indices='all', check=True):

    return _aux_getter_big_attribute_from_small(item, 'chain_name', 'group', indices)

def get_chain_type_from_group (item, indices='all', structure_indices='all', check=True):

    return _aux_getter_big_attribute_from_small(item, 'chain_type', 'group', indices)

def get_molecule_index_from_group (item, indices='all', structure_indices='all', check=True):

    return _aux_getter_big_index_from_small(item, 'molecule_index', 'group', indices)

def get_molecule_id_from_group (item, indices='all', structure_indices='all', check=True):

    return _aux_getter_big_attribute_from_small(item, 'molecule_id', 'group', indices)

def get_molecule_name_from_group (item, indices='all', structure_indices='all', check=True):

    return _aux_getter_big_attribute_from_small(item, 'molecule_name', 'group', indices)

def get_molecule_type_from_group (item, indices='all', structure_indices='all', check=True):

    return _aux_getter_big_attribute_from_small(item, 'molecule_type', 'group', indices)

def get_entity_index_from_group (item, indices='all', structure_indices='all', check=True):

    return _aux_getter_big_index_from_small(item, 'entity_index', 'group', indices)

def get_entity_id_from_group (item, indices='all', structure_indices='all', check=True):

    return _aux_getter_big_attribute_from_small(item, 'entity_id', 'group', indices)

def get_entity_name_from_group (item, indices='all', structure_indices='all', check=True):

    return _aux_getter_big_attribute_from_small(item, 'entity_name', 'group', indices)

def get_entity_type_from_group (item, indices='all', structure_indices='all', check=True):

    return _aux_getter_big_attribute_from_small(item, 'entity_type', 'group', indices)

def get_n_atoms_from_group (item, indices='all', structure_indices='all', check=True):

    return _aux_n_small_from_big(item, 'n_atoms', 'group', indices)

def get_n_groups_from_group (item, indices='all', structure_indices='all', check=True):

    return _aux_n(item, 'group', indices)

def get_n_components_from_group (item, indices='all', structure_indices='all', check=True):

    return _aux_n_big_from_small(item, 'n_components', 'group', indices)

def get_n_molecules_from_group (item, indices='all', structure_indices='all', check=True):

    return _aux_n_big_from_small(item, 'n_molecules', 'group', indices)

def get_n_chains_from_group (item, indices='all', structure_indices='all', check=True):

    return _aux_n_big_from_small(item, 'n_chains', 'group', indices)

def get_n_entities_from_group (item, indices='all', structure_indices='all', check=True):

    return _aux_n_big_from_small(item, 'n_entities', 'group', indices)

## component

def get_atom_index_from_component(item, indices='all', structure_indices='all', check=True):

    return _aux_getter_small_index_from_big(item, 'atom_index', 'component', indices)

def get_atom_id_from_component(item, indices='all', structure_indices='all', check=True):

    return _aux_getter_small_attribute_from_big(item, 'atom_id', 'component', indices)

def get_atom_name_from_component(item, indices='all', structure_indices='all', check=True):

    return _aux_getter_small_attribute_from_big(item, 'atom_name', 'component', indices)

def get_atom_type_from_component(item, indices='all', structure_indices='all', check=True):

    return _aux_getter_small_attribute_from_big(item, 'atom_type', 'component', indices)

def get_group_index_from_component(item, indices='all', structure_indices='all', check=True):

    return _aux_getter_small_index_from_big(item, 'group_index', 'component', indices)

def get_group_id_from_component(item, indices='all', structure_indices='all', check=True):

    return _aux_getter_small_attribute_from_big(item, 'group_id', 'component', indices)

def get_group_name_from_component(item, indices='all', structure_indices='all', check=True):

    return _aux_getter_small_attribute_from_big(item, 'group_name', 'component', indices)

def get_group_type_from_component(item, indices='all', structure_indices='all', check=True):

    return _aux_getter_small_attribute_from_big(item, 'group_type', 'component', indices)

def get_component_index_from_component (item, indices='all', structure_indices='all', check=True):

    return _aux_getter_index(item, 'component', indices)

#def get_component_id_from_component (item, indices='all', structure_indices='all', check=True):
#
#    return _aux_getter_attribute(item, 'id', 'component', indices)
#
#def get_component_name_from_component (item, indices='all', structure_indices='all', check=True):
#
#    return _aux_getter_attribute(item, 'name', 'component', indices)
#
#def get_component_type_from_component (item, indices='all', structure_indices='all', check=True):
#
#    return _aux_getter_attribute(item, 'type', 'component', indices)

def get_chain_index_from_component (item, indices='all', structure_indices='all', check=True):

    return _aux_getter_big_index_from_small(item, 'chain_index', 'component', indices)

def get_chain_id_from_component (item, indices='all', structure_indices='all', check=True):

    return _aux_getter_big_attribute_from_small(item, 'chain_id', 'component', indices)

def get_chain_name_from_component (item, indices='all', structure_indices='all', check=True):

    return _aux_getter_big_attribute_from_small(item, 'chain_name', 'component', indices)

def get_chain_type_from_component (item, indices='all', structure_indices='all', check=True):

    return _aux_getter_big_attribute_from_small(item, 'chain_type', 'component', indices)

def get_molecule_index_from_component (item, indices='all', structure_indices='all', check=True):

    return _aux_getter_big_index_from_small(item, 'molecule_index', 'component', indices)

def get_molecule_id_from_component (item, indices='all', structure_indices='all', check=True):

    return _aux_getter_big_attribute_from_small(item, 'molecule_id', 'component', indices)

def get_molecule_name_from_component (item, indices='all', structure_indices='all', check=True):

    return _aux_getter_big_attribute_from_small(item, 'molecule_name', 'component', indices)

def get_molecule_type_from_component (item, indices='all', structure_indices='all', check=True):

    return _aux_getter_big_attribute_from_small(item, 'molecule_type', 'component', indices)

def get_entity_index_from_component (item, indices='all', structure_indices='all', check=True):

    return _aux_getter_big_index_from_small(item, 'entity_index', 'component', indices)

def get_entity_id_from_component (item, indices='all', structure_indices='all', check=True):

    return _aux_getter_big_attribute_from_small(item, 'entity_id', 'component', indices)

def get_entity_name_from_component (item, indices='all', structure_indices='all', check=True):

    return _aux_getter_big_attribute_from_small(item, 'entity_name', 'component', indices)

def get_entity_type_from_component (item, indices='all', structure_indices='all', check=True):

    return _aux_getter_big_attribute_from_small(item, 'entity_type', 'component', indices)

def get_n_atoms_from_component (item, indices='all', structure_indices='all', check=True):

    return _aux_n_small_from_big(item, 'n_atoms', 'component', indices)

def get_n_groups_from_component (item, indices='all', structure_indices='all', check=True):

    return _aux_n_small_from_big(item, 'n_groups', 'component', indices)

def get_n_components_from_component (item, indices='all', structure_indices='all', check=True):

    return _aux_n(item, 'component', indices)

def get_n_molecules_from_component (item, indices='all', structure_indices='all', check=True):

    return _aux_n_big_from_small(item, 'n_molecules', 'component', indices)

def get_n_chains_from_component (item, indices='all', structure_indices='all', check=True):

    return _aux_n_big_from_small(item, 'n_chains', 'component', indices)

def get_n_entities_from_component (item, indices='all', structure_indices='all', check=True):

    return _aux_n_big_from_small(item, 'n_entities', 'component', indices)

## molecule

def get_atom_index_from_molecule(item, indices='all', structure_indices='all', check=True):

    return _aux_getter_small_index_from_big(item, 'atom_index', 'molecule', indices)

def get_atom_id_from_molecule(item, indices='all', structure_indices='all', check=True):

    return _aux_getter_small_attribute_from_big(item, 'atom_id', 'molecule', indices)

def get_atom_name_from_molecule(item, indices='all', structure_indices='all', check=True):

    return _aux_getter_small_attribute_from_big(item, 'atom_name', 'molecule', indices)

def get_atom_type_from_molecule(item, indices='all', structure_indices='all', check=True):

    return _aux_getter_small_attribute_from_big(item, 'atom_type', 'molecule', indices)

def get_group_index_from_molecule(item, indices='all', structure_indices='all', check=True):

    return _aux_getter_small_index_from_big(item, 'group_index', 'molecule', indices)

def get_group_id_from_molecule(item, indices='all', structure_indices='all', check=True):

    return _aux_getter_small_attribute_from_big(item, 'group_id', 'molecule', indices)

def get_group_name_from_molecule(item, indices='all', structure_indices='all', check=True):

    return _aux_getter_small_attribute_from_big(item, 'group_name', 'molecule', indices)

def get_group_type_from_molecule(item, indices='all', structure_indices='all', check=True):

    return _aux_getter_small_attribute_from_big(item, 'group_type', 'molecule', indices)

def get_component_index_from_molecule (item, indices='all', structure_indices='all', check=True):

    return _aux_getter_small_index_from_big(item, 'component_index', 'molecule', indices)

def get_component_id_from_molecule (item, indices='all', structure_indices='all', check=True):

    return _aux_getter_small_attribute_from_big(item, 'component_id', 'molecule', indices)

def get_component_name_from_molecule (item, indices='all', structure_indices='all', check=True):

    return _aux_getter_small_attribute_from_big(item, 'component_name', 'molecule', indices)

def get_component_type_from_molecule (item, indices='all', structure_indices='all', check=True):

    return _aux_getter_small_attribute_from_big(item, 'component_type', 'molecule', indices)

def get_chain_index_from_molecule (item, indices='all', structure_indices='all', check=True):

    return _aux2_getter_small_index_from_big(item, 'chain_index', 'molecule', indices)

def get_chain_id_from_molecule (item, indices='all', structure_indices='all', check=True):

    return _aux_getter_small_attribute_from_big(item, 'chain_id', 'molecule', indices)

def get_chain_name_from_molecule (item, indices='all', structure_indices='all', check=True):

    return _aux_getter_small_attribute_from_big(item, 'chain_name', 'molecule', indices)

def get_chain_type_from_molecule (item, indices='all', structure_indices='all', check=True):

    return _aux_getter_small_attribute_from_big(item, 'chain_type', 'molecule', indices)

def get_molecule_index_from_molecule (item, indices='all', structure_indices='all', check=True):

    return _aux_getter_index(item, 'molecule', indices)

#def get_molecule_id_from_molecule (item, indices='all', structure_indices='all', check=True):
#
...
def get_entity_index_from_molecule (item, indices='all', structure_indices='all', check=True):

    return _aux_getter_big_index_from_small(item, 'entity_index', 'molecule', indices)

def get_entity_id_from_molecule (item, indices='all', structure_indices='all', check=True):

    return _aux_getter_big_attribute_from_small(item, 'entity_id', 'molecule', indices)

def get_entity_name_from_molecule (item, indices='all', structure_indices='all', check=True):

    return _aux_getter_big_attribute_from_small(item, 'entity_name', 'molecule', indices)

def get_entity_type_from_molecule (item, indices='all', structure_indices='all', check=True):

    return _aux_getter_big_attribute_from_small(item, 'entity_type', 'molecule', indices)

def get_n_atoms_from_molecule (item, indices='all', structure_indices='all', check=True):

    return _aux_n_small_from_big(item, 'n_atoms', 'molecule', indices)

def get_n_groups_from_molecule (item, indices='all', structure_indices='all', check=True):

    return _aux_n_small_from_big(item, 'n_groups', 'molecule', indices)

def get_n_components_from_molecule (item, indices='all', structure_indices='all', check=True):

    return _aux_n_small_from_big(item, 'n_components', 'molecule', indices)

def get_n_molecules_from_molecule (item, indices='all', structure_indices='all', check=True):

    return _aux_n(item, 'molecule', indices)

def get_n_chains_from_molecule (item, indices='all', structure_indices='all', check=True):

    return _aux_n_small_from_big(item, 'n_chains', 'molecule', indices)

def get_n_entities_from_molecule (item, indices='all', structure_indices='all', check=True):

    return _aux_n_big_from_small(item, 'n_entities', 'molecule', indices)

## chain

def get_atom_index_from_chain(item, indices='all', structure_indices='all', check=True):

    return _aux_getter_small_index_from_big(item, 'atom_index', 'chain', indices)

def get_atom_id_from_chain(item, indices='all', structure_indices='all', check=True):

    return _aux_getter_small_attribute_from_big(item, 'atom_id', 'chain', indices)

def get_atom_name_from_chain(item, indices='all', structure_indices='all', check=True):

    return _aux_getter_small_attribute_from_big(item, 'atom_name', 'chain', indices)

def get_atom_type_from_chain(item, indices='all', structure_indices='all', check=True):

    return _aux_getter_small_attribute_from_big(item, 'atom_type', 'chain', indices)

def get_group_index_from_chain(item, indices='all', structure_indices='all', check=True):

    return _aux_getter_small_index_from_big(item, 'group_index', 'chain', indices)

def get_group_id_from_chain(item, indices='all', structure_indices='all', check=True):

    return _aux_getter_small_attribute_from_big(item, 'group_id', 'chain', indices)

def get_group_name_from_chain(item, indices='all', structure_indices='all', check=True):

    return _aux_getter_small_attribute_from_big(item, 'group_name', 'chain', indices)

def get_group_type_from_chain(item, indices='all', structure_indices='all', check=True):

    return _aux_getter_small_attribute_from_big(item, 'group_type', 'chain', indices)

def get_component_index_from_chain (item, indices='all', structure_indices='all', check=True):

    return _aux_getter_small_index_from_big(item, 'component_index', 'chain', indices)

def get_component_id_from_chain (item, indices='all', structure_indices='all', check=True):

    return _aux_getter_small_attribute_from_big(item, 'component_id', 'chain', indices)

def get_component_name_from_chain (item, indices='all', structure_indices='all', check=True):

    return _aux_getter_small_attribute_from_big(item, 'component_name', 'chain', indices)

def get_component_type_from_chain (item, indices='all', structure_indices='all', check=True):

    return _aux_getter_small_attribute_from_big(item, 'component_type', 'chain', indices)

def get_chain_index_from_chain (item, indices='all', structure_indices='all', check=True):
...
def get_molecule_index_from_chain (item, indices='all', structure_indices='all', check=True):

    return _aux2_getter_small_index_from_big(item, 'molecule_index', 'chain', indices)

def get_molecule_id_from_chain (item, indices='all', structure_indices='all', check=True):

    return _aux_getter_small_attribute_from_big(item, 'molecule_id', 'chain', indices)

def get_molecule_name_from_chain (item, indices='all', structure_indices='all', check=True):

    return _aux_getter_small_attribute_from_big(item, 'molecule_name', 'chain', indices)

def get_molecule_type_from_chain (item, indices='all', structure_indices='all', check=True):

    return _aux_getter_small_attribute_from_big(item, 'molecule_type', 'chain', indices)

def get_entity_index_from_chain (item, indices='all', structure_indices='all', check=True):

    return _aux2_getter_small_index_from_big(item, 'entity_index', 'chain', indices)

def get_entity_id_from_chain (item, indices='all', structure_indices='all', check=True):

    return _aux_getter_small_attribute_from_big(item, 'entity_id', 'chain', indices)

def get_entity_name_from_chain (item, indices='all', structure_indices='all', check=True):

    return _aux_getter_small_attribute_from_big(item, 'entity_name', 'chain', indices)

def get_entity_type_from_chain (item, indices='all', structure_indices='all', check=True):

    return _aux_getter_small_attribute_from_big(item, 'entity_type', 'chain', indices)

def get_n_atoms_from_chain (item, indices='all', structure_indices='all', check=True):

    return _aux_n_small_from_big(item, 'n_atoms', 'chain', indices)

def get_n_groups_from_chain (item, indices='all', structure_indices='all', check=True):

    return _aux_n_small_from_big(item, 'n_groups', 'chain', indices)

def get_n_components_from_chain (item, indices='all', structure_indices='all', check=True):

    return _aux_n_small_from_big(item, 'n_components', 'chain', indices)

def get_n_molecules_from_chain (item, indices='all', structure_indices='all', check=True):

    return _aux_n_small_from_big(item, 'n_molecules', 'chain', indices)

def get_n_chains_from_chain (item, indices='all', structure_indices='all', check=True):

    return _aux_n(item, 'chain', indices)

def get_n_entities_from_chain (item, indices='all', structure_indices='all', check=True):

    return _aux_n_big_from_small(item, 'n_entities', 'chain', indices)

## entity

def get_atom_index_from_entity(item, indices='all', structure_indices='all', check=True):

    return _aux_getter_small_index_from_big(item, 'atom_index', 'entity', indices)

def get_atom_id_from_entity(item, indices='all', structure_indices='all', check=True):

    return _aux_getter_small_attribute_from_big(item, 'atom_id', 'entity', indices)

def get_atom_name_from_entity(item, indices='all', structure_indices='all', check=True):

    return _aux_getter_small_attribute_from_big(item, 'atom_name', 'entity', indices)

def get_atom_type_from_entity(item, indices='all', structure_indices='all', check=True):

    return _aux_getter_small_attribute_from_big(item, 'atom_type', 'entity', indices)

def get_group_index_from_entity(item, indices='all', structure_indices='all', check=True):

    return _aux_getter_small_index_from_big(item, 'group_index', 'entity', indices)

def get_group_id_from_entity(item, indices='all', structure_indices='all', check=True):

    return _aux_getter_small_attribute_from_big(item, 'group_id', 'entity', indices)

def get_group_name_from_entity(item, indices='all', structure_indices='all', check=True):

    return _aux_getter_small_attribute_from_big(item, 'group_name', 'entity', indices)

def get_group_type_from_entity(item, indices='all', structure_indices='all', check=True):

    return _aux_getter_small_attribute_from_big(item, 'group_type', 'entity', indices)

def get_component_index_from_entity (item, indices='all', structure_indices='all', check=True):

    return _aux_getter_small_index_from_big(item, 'component_index', 'entity', indices)

def get_component_id_from_entity (item, indices='all', structure_indices='all', check=True):

    return _aux_getter_small_attribute_from_big(item, 'component_id', 'entity', indices)

def get_component_name_from_entity (item, indices='all', structure_indices='all', check=True):

    return _aux_getter_small_attribute_from_big(item, 'component_name', 'entity', indices)

def get_component_type_from_entity (item, indices='all', structure_indices='all', check=True):

    return _aux_getter_small_attribute_from_big(item, 'component_type', 'entity', indices)

def get_chain_index_from_entity (item, indices='all', structure_indices='all', check=True):

    return _aux2_getter_small_index_from_big(item, 'chain_index', 'entity', indices)

def get_chain_id_from_entity (item, indices='all', structure_indices='all', check=True):

    return _aux_getter_small_attribute_from_big(item, 'chain_id', 'entity', indices)

def get_chain_name_from_entity (item, indices='all', structure_indices='all', check=True):

    return _aux_getter_small_attribute_from_big(item, 'chain_name', 'entity', indices)

def get_chain_type_from_entity (item, indices='all', structure_indices='all', check=True):

    return _aux_getter_small_attribute_from_big(item, 'chain_type', 'entity', indices)

def get_molecule_index_from_entity (item, indices='all', structure_indices='all', check=True):

    return _aux_getter_small_index_from_big(item, 'molecule_index', 'entity', indices)

def get_molecule_id_from_entity (item, indices='all', structure_indices='all', check=True):

    return _aux_getter_small_attribute_from_big(item, 'molecule_id', 'entity', indices)

def get_molecule_name_from_entity (item, indices='all', structure_indices='all', check=True):

    return _aux_getter_small_attribute_from_big(item, 'molecule_name', 'entity', indices)

def get_molecule_type_from_entity (item, indices='all', structure_indices='all', check=True):

    return _aux_getter_small_attribute_from_big(item, 'molecule_type', 'entity', indices)

def get_entity_index_from_entity (item, indices='all', structure_indices='all', check=True):
...
def get_n_atoms_from_entity (item, indices='all', structure_indices='all', check=True):

    return _aux_n_small_from_big(item, 'n_atoms', 'entity', indices)

def get_n_groups_from_entity (item, indices='all', structure_indices='all', check=True):

    return _aux_n_small_from_big(item, 'n_groups', 'entity', indices)

def get_n_components_from_entity (item, indices='all', structure_indices='all', check=True):

    return _aux_n_small_from_big(item, 'n_components', 'entity', indices)

def get_n_molecules_from_entity (item, indices='all', structure_indices='all', check=True):

    return _aux_n_small_from_big(item, 'n_molecules', 'entity', indices)

def get_n_chains_from_entity (item, indices='all', structure_indices='all', check=True):

    return _aux_n_small_from_big(item, 'n_chains', 'entity', indices)

def get_n_entities_from_entity (item, indices='all', structure_indices='all', check=True):
...
def get_n_aminoacids_from_system (item, indices='all', structure_indices='all', check=True):

    from molsysmt.basic import get

    group_types = get(item, indices='all', target='group', group_type=True)
    return (group_types=='aminoacid').sum()

def get_n_nucleotides_from_system (item, indices='all', structure_indices='all', check=True):
...
def get_n_ions_from_system (item, indices='all', structure_indices='all', check=True):

    from molsysmt.basic import get

    molecule_types = get(item, indices='all', target='molecule', molecule_type=True)
    return (molecule_types=='ion').sum()

def get_n_waters_from_system (item, indices='all', structure_indices='all', check=True):
...
def get_n_small_molecules_from_system (item, indices='all', structure_indices='all', check=True):
...
def get_n_peptides_from_system (item, indices='all', structure_indices='all', check=True):
...
def get_n_proteins_from_system (item, indices='all', structure_indices='all', check=True):
...
def get_n_dnas_from_system (item, indices='all', structure_indices='all', check=True):
...
def get_n_rnas_from_system (item, indices='all', structure_indices='all', check=True):
...
def get_n_lipids_from_system (item, indices='all', structure_indices='all', check=True):
...
def get_coordinates_from_system(item, indices='all', structure_indices='all', check=True):
    ...
    from molsysmt.basic import get
    return get(item, structure_indices=structure_indices, target='atom')
...
def get_frame_from_system(item, indices='all', structure_indices='all', check=True):

    from molsysmt.basic import get
    return get(item, structure_indices=structure_indices, target='atom', frame=True)
...
def get_bonded_atoms_from_system(item, indices='all', structure_indices='all', check=True):
    ...
    from molsysmt.basic import get
    return get(item, indices='all', target='atoms', bonded_atoms=True)

def get_bond_index_from_system(item, indices='all', structure_indices='all', check=True):

    from molsysmt.basic import get
    return get(item, indices='all', target='atoms', bond_index=True)

def get_inner_bonded_atoms_from_system(item, indices='all', structure_indices='all', check=True):

    from molsysmt.basic import get
    return get(item, indices='all', target='bond', atom_index=True)

def get_inner_bond_index_from_system(item, indices='all', structure_indices='all', check=True):

    from molsysmt.basic import get
    return get(item, indices='all', target='bond', bond_index=True)

## bond

def get_bond_index_from_bond(item, indices='all', structure_indices='all', check=True):
...
def get_n_bonds_from_bond(item, indices='all', structure_indices='all', check=True):
