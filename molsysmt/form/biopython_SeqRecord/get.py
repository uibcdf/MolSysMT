...
@digest(form=form)
def get_coordinates_from_atom(item, indices='all', digest=True):
...
@digest(form=form)
def get_box_from_system(item, digest=True):

    raise NotWithThisFormError()

@digest(form=form)
def get_time_from_system(item, digest=True):

    raise NotWithThisFormError()

@digest(form=form)
def get_structure_id_from_system(item, digest=True):
