...
def get_entity_id_from_atom(item, indices='all', digest=True):
    ...
    tmp_indices = get_atom_index_from_atom(item, indices=indices, structure_indices=structure_indices, digest=False)
...
def get_n_atoms_from_atom(item, indices='all', digest=True):
    ...
    if is_all(indices):
        return get_n_atoms_from_system(item, digest=False)
    else:
...
def get_inner_bond_index_from_atom(item, indices='all', digest=True):
    ...
    if is_all(indices):
        output = get_bond_index_from_bond(item, digest=False)
    else:
...
def get_inner_bonded_atoms_from_atom(item, indices='all', digest=True):
    ...
    if is_all(indices):
        ...
        output = get_atom_index_from_bond(item, indices='all', digest=False)

    else:

        bond_indices = get_inner_bond_index_from_atom (item, indices=indices, digest=False)
        output = get_atom_index_from_bond(item, indices=bond_indices, digest=False)
        del(bond_indices)
...
def get_atom_id_from_group(item, indices='all', digest=True):
    ...
    output=[]
    tmp_indices = get_atom_index_from_group(item, indices=indices, digest=False)
    for aux_indices in tmp_indices:
        output.append(get_atom_id_from_atom(item, indices=aux_indices, digest=False))
    output = np.array(output)
    return output
...
def get_atom_name_from_group(item, indices='all', digest=True):
    ...
    tmp_indices = get_atom_index_from_group(item, indices=indices, digest=False)
    for aux_indices in tmp_indices:
        output.append(get_atom_name_from_atom(item, indices=aux_indices, digest=False))
    output = np.array(output, dtype=object)
    return output
...
def get_atom_type_from_group(item, indices='all', digest=True):
    ...
    tmp_indices = get_atom_index_from_group(item, indices=indices, digest=False)
    for aux_indices in tmp_indices:
        output.append(get_atom_type_from_atom(item, indices=aux_indices, digest=False))
    output = np.array(output, dtype=object)
    return output
...
def get_group_id_from_group(item, indices='all', digest=True):
    ...
    tmp_indices = get_group_index_from_group(item, indices=indices, digest=False)
...
def get_n_groups_from_group(item, indices='all', digest=True):
    ...
    if is_all(indices):
        return get_n_groups_from_system (item, digest=False)
    else:
        output = get_group_index_from_group (item, indices=indices, digest=False)
        return output.shape[0]
...
def get_atom_index_from_component(item, indices='all', digest=True):
    ...
    output = []
    if is_all(indices):
        n_indices = get_n_components_from_system(item, digest=False)
        indices = range(n_indices)
    for ii in indices:
...
def get_atom_id_from_component(item, indices='all', digest=True):
    ...
    output=[]
    tmp_indices = get_atom_index_from_component(item, indices=indices, digest=False)
    for aux_indices in tmp_indices:
        output.append(get_atom_id_from_atom(item, indices=aux_indices, digest=False))
    output = np.array(output, dtype=object)
    return output
...
def get_group_id_from_component(item, indices='all', digest=True):
    ...
    output=[]
    tmp_indices = get_group_index_from_component(item, indices=indices, digest=False)
    for aux_indices in tmp_indices:
        output.append(get_group_id_from_group(item, indices=aux_indices, digest=False))
    output = np.array(output, dtype=object)
    return output
...
def get_group_name_from_component(item, indices='all', digest=True):
    ...
    tmp_indices = get_group_index_from_component(item, indices=indices, digest=False)
    for aux_indices in tmp_indices:
        output.append(get_group_name_from_group(item, indices=aux_indices, digest=False))
    output = np.array(output, dtype=object)
    return output
...
def get_group_type_from_component(item, indices='all', digest=True):
    ...
    tmp_indices = get_group_index_from_component(item, indices=indices, digest=False)
    for aux_indices in tmp_indices:
        output.append(get_group_type_from_group(item, indices=aux_indices, digest=False))
    output = np.array(output, dtype=object)
    return output
...
def get_component_id_from_component(item, indices='all', digest=True):
    ...
    tmp_indices = get_component_index_from_component(item, indices=indices, digest=False)
...
def get_atom_index_from_molecule(item, indices='all', digest=True):
    ...
    output = []
    if is_all(indices):
        n_indices = get_n_molecules_from_system(item, digest=False)
        indices = range(n_indices)
    for ii in indices:
...
def get_atom_id_from_molecule(item, indices='all', digest=True):
    ...
    output=[]
    tmp_indices = get_atom_index_from_molecule(item, indices=indices, digest=False)
    for aux_indices in tmp_indices:
        output.append(get_atom_id_from_atom(item, indices=aux_indices, digest=False))
    output = np.array(output)
    return output
...
def get_group_id_from_molecule(item, indices='all', digest=True):
    ...
    output=[]
    tmp_indices = get_group_index_from_molecule(item, indices=indices, digest=False)
    for aux_indices in tmp_indices:
        output.append(get_group_id_from_group(item, indices=aux_indices, digest=False))
    output = np.array(output, dtype=object)
    return output
...
def get_component_id_from_molecule(item, indices='all', digest=True):
    ...
    output=[]
    tmp_indices = get_component_index_from_molecule(item, indices=indices, digest=False)
    for aux_indices in tmp_indices:
        output.append(get_component_id_from_component(item, indices=aux_indices, digest=False))
    output = np.array(output)
    return output
...
def get_component_name_from_molecule(item, indices='all', digest=True):
    ...
    tmp_indices = get_component_index_from_molecule(item, indices=indices, digest=False)
    for aux_indices in tmp_indices:
        output.append(get_component_name_from_component(item, indices=aux_indices, digest=False))
    output = np.array(output)
    return output
...
def get_component_type_from_molecule(item, indices='all', digest=True):
    ...
    tmp_indices = get_component_index_from_molecule(item, indices=indices, digest=False)
    for aux_indices in tmp_indices:
        output.append(get_component_type_from_component(item, indices=aux_indices, digest=False))
    output = np.array(output)
    return output
...
def get_chain_id_from_molecule(item, indices='all', digest=True):
    ...
    output=[]
    tmp_indices = get_chain_index_from_molecule(item, indices=indices, digest=False)
    for aux_indices in tmp_indices:
        output.append(get_chain_id_from_chain(item, indices=aux_indices, digest=False))
    output = np.array(output)
    return output
...
def get_chain_name_from_molecule(item, indices='all', digest=True):
    ...
    tmp_indices = get_chain_index_from_molecule(item, indices=indices, digest=False)
    for aux_indices in tmp_indices:
        output.append(get_chain_name_from_chain(item, indices=aux_indices, digest=False))
    output = np.array(output)
    return output
...
def get_chain_type_from_molecule(item, indices='all', digest=True):
    ...
    tmp_indices = get_chain_index_from_molecule(item, indices=indices, digest=False)
    for aux_indices in tmp_indices:
        output.append(get_chain_type_from_chain(item, indices=aux_indices, digest=False))
    output = np.array(output)
    return output
...
def get_molecule_id_from_molecule(item, indices='all', digest=True):
    ...
    tmp_indices = get_molecule_index_from_molecule(item, indices=indices, digest=False)
...
def get_n_molecules_from_molecule(item, indices='all', digest=True):
    ...
    if is_all(indices):
        ...
        output = get_molecule_index_from_molecule (item, indices=indices, digest=False)
        return output.shape[0]
...
def get_atom_id_from_chain(item, indices='all', digest=True):
    ...
    output=[]
    tmp_indices = get_atom_index_from_chain(item, indices=indices, digest=False)
    for aux_indices in tmp_indices:
        output.append(get_atom_id_from_atom(item, indices=aux_indices, digest=False))
    output = np.array(output)
    return output
...
def get_group_id_from_chain(item, indices='all', digest=True):
    ...
    output=[]
    tmp_indices = get_group_index_from_chain(item, indices=indices, digest=False)
    for aux_indices in tmp_indices:
        output.append(get_group_id_from_group(item, indices=aux_indices, digest=False))
    output = np.array(output)
    return output
...
def get_component_id_from_chain(item, indices='all', digest=True):
    ...
    output=[]
    tmp_indices = get_component_index_from_chain(item, indices=indices, digest=False)
    for aux_indices in tmp_indices:
        output.append(get_component_id_from_component(item, indices=aux_indices, digest=False))
    output = np.array(output)
    return output
...
def get_chain_id_from_chain(item, indices='all', digest=True):
    ...
    tmp_indices = get_chain_index_from_chain(item, indices=indices, digest=False)
...
def get_molecule_id_from_chain(item, indices='all', digest=True):
    ...
    output=[]
    tmp_indices = get_molecule_index_from_chain(item, indices=indices, digest=False)
    for aux_indices in tmp_indices:
...
def get_molecule_type_from_chain(item, indices='all', digest=True):
    ...
    tmp_indices = get_molecule_index_from_chain(item, indices=indices, digest=False)
    for aux_indices in tmp_indices:
        output.append(get_molecule_type_from_molecule(item, indices=aux_indices, digest=False))
    output = np.array(output, dtype=object)
    return output
...
def get_entity_id_from_chain(item, indices='all', digest=True):
    ...
    output=[]
    tmp_indices = get_entity_index_from_chain(item, indices=indices, digest=False)
    for aux_indices in tmp_indices:
        output.append(get_entity_id_from_entity(item, indices=aux_indices, digest=False))
    output = np.array(output)
    return output
...
def get_entity_name_from_chain(item, indices='all', digest=True):
    ...
    tmp_indices = get_entity_index_from_chain(item, indices=indices, digest=False)
    for aux_indices in tmp_indices:
        output.append(get_entity_name_from_entity(item, indices=aux_indices, digest=False))
    output = np.array(output)
    return output
...
def get_entity_type_from_chain(item, indices='all', digest=True):
    ...
    tmp_indices = get_entity_index_from_chain(item, indices=indices, digest=False)
    for aux_indices in tmp_indices:
        output.append(get_entity_type_from_entity(item, indices=aux_indices, digest=False))
    output = np.array(output)
    return output
...
def get_n_chains_from_chain(item, indices='all', digest=True):
    ...
    if is_all(indices):
        ...
        output = get_chain_index_from_chain (item, indices=indices, digest=False)
        output = np.unique(output)
        return output.shape[0]
...
def get_atom_index_from_entity(item, indices='all', digest=True):
    ...
    output = []
    if is_all(indices):
        n_indices = get_n_entities_from_system(item, digest=False)
        indices = range(n_indices)
    for ii in indices:
...
def get_atom_id_from_entity(item, indices='all', digest=True):
    ...
    output=[]
    tmp_indices = get_atom_index_from_entity(item, indices=indices, digest=False)
    for aux_indices in tmp_indices:
        output.append(get_atom_id_from_atom(item, indices=aux_indices, digest=False))
    output = np.array(output, dtype=object)
    return output
...
def get_atom_type_from_entity(item, indices='all', digest=True):
    ...
    tmp_indices = get_atom_index_from_entity(item, indices=indices, digest=False)
    for aux_indices in tmp_indices:
        output.append(get_atom_type_from_atom(item, indices=aux_indices, structure_indices=structure_indices, digest=False))
    output = np.array(output, dtype=object)
    return output
...
def get_group_id_from_entity(item, indices='all', digest=True):
    ...
    output=[]
    tmp_indices = get_group_index_from_entity(item, indices=indices, digest=False)
    for aux_indices in tmp_indices:
        output.append(get_group_id_from_group(item, indices=aux_indices, digest=False))
    output = np.array(output, dtype=object)
    return output
...
def get_component_id_from_entity(item, indices='all', digest=True):
    ...
    output=[]
    tmp_indices = get_component_index_from_entity(item, indices=indices, digest=False)
    for aux_indices in tmp_indices:
        output.append(get_component_id_from_component(item, indices=aux_indices, digest=False))
    output = np.array(output)
    return output
...
def get_chain_id_from_entity(item, indices='all', digest=True):
    ...
    output=[]
    tmp_indices = get_chain_index_from_entity(item, indices=indices, digest=False)
    for aux_indices in tmp_indices:
        output.append(get_chain_id_from_chain(item, indices=aux_indices, digest=False))
    output = np.array(output)
    return output
...
def get_molecule_id_from_entity(item, indices='all', digest=True):
    ...
    output=[]
    tmp_indices = get_molecule_index_from_entity(item, indices=indices, digest=False)
    for aux_indices in tmp_indices:
        output.append(get_molecule_id_from_molecule(item, indices=aux_indices, digest=False))
    output = np.array(output)
    return output
...
def get_molecule_name_from_entity(item, indices='all', digest=True):
    ...
    tmp_indices = get_molecule_index_from_entity(item, indices=indices, digest=False)
    for aux_indices in tmp_indices:
        output.append(get_molecule_name_from_molecule(item, indices=aux_indices, digest=False))
    output = np.array(output)
    return output
...
def get_entity_id_from_entity(item, indices='all', digest=True):
    ...
    tmp_indices = get_entity_index_from_entity(item, indices=indices, digest=False)
...
def get_n_entities_from_entity(item, indices='all', digest=True):
    ...
    if is_all(indices):
        ...
        output = get_entity_index_from_entity (item, indices=indices, digest=False)
        output = np.unique(output)
        return output.shape[0]
