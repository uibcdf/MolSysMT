...
def to_mdtraj_Trajectory(item, atom_indices='all', structure_indices='all', check=True):
    ...
    from ..string_pdb_text import to_mdtraj_Trajectory as string_pdb_text_to_mdtraj_Trajectory

    tmp_item = to_string_pdb_text(item, check=False)
    tmp_item = string_pdb_text_to_mdtraj_Trajectory(tmp_item, atom_indices=atom_indices,
