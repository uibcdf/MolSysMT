...
def to_molsysmt_MolSys(item, atom_indices='all', structure_indices='all', bioassembly_name=None, digest=True):
    ...
    tmp_item = MolSys()
    tmp_item.topology = to_molsysmt_Topology(item, atom_indices=atom_indices, structure_indices=structure_indices, digest=False)
    ...
    if bioassembly_name is not None:
        ...
        bioassemblies = get_bioassemblies_from_system(item, digest=False)
        tmp_item = make_bioassembly(tmp_item, bioassembly=bioassemblies[bioassembly_name], digest=False)
