def from_pdbfixer_PDBFixer (item, molecular_system, atom_indices='all', frame_indices='all'):
    ...
    tmp_item = MolSys()
    tmp_item.topology, _ = pdbfixer_PDBFixer_to_molsysmt_Topology(item, molecular_system, atom_indices=atom_indices, frame_indices=frame_indices)
    tmp_item.trajectory, _ = pdbfixer_PDBFixer_to_molsysmt_Trajectory(item, molecular_system, atom_indices=atom_indices, frame_indices=frame_indices)
...
def to_pdbfixer_PDBFixer (item, molecular_system, atom_indices='all', frame_indices='all'):
    ...
    tmp_item, _ = molsysmt_MolSys_to_pdb(item, molecular_system, atom_indices=atom_indices, frame_indices=frame_indices, output_filename='.pdb')
