def to_pdb(item, molecular_system, atom_indices='all', frame_indices='all', output_filename=None):
    ...
    tmp_item, tmp_molecular_system = openmm_Topology_to_pdb(tmp_item, tmp_molecular_system, atom_indices=atom_indices, frame_indices=frame_indices, output_filename=output_filename)

    return tmp_item, tmp_molecular_system

def from_pdb(item, molecular_system, atom_indices='all', frame_indices='all'):
    ...
    tmp_item = MolSys()
    tmp_item.topology, _ = pdb_to_molsysmt_Topology(item, molecular_system, atom_indices=atom_indices, frame_indices=frame_indices)
    tmp_item.trajectory, _ = pdb_to_molsysmt_Trajectory(item, molecular_system, atom_indices=atom_indices, frame_indices=frame_indices)
