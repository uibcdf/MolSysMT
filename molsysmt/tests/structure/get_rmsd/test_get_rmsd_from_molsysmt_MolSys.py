...
def test_get_rmsd_molsysmt_MolSys_1():
    ...
    rmsd = msm.structure.get_rmsd(molsys, selection='backbone', structure_indices=100, reference_frame_index=0)
...
def test_get_rmsd_molsysmt_MolSys_2():
    molsys = msm.convert(msm.demo['pentalanine']['traj.h5'], to_form='molsysmt.MolSys')
    rmsd = msm.structure.get_rmsd(molsys, selection='backbone', structure_indices='all', reference_frame_index=0)
