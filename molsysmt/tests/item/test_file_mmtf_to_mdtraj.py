...
###    traj = file_mmtf.load_mmtf(str(mmtf_file_paths[0]))
###    assert traj.n_atoms == 4257
###    assert traj.n_frames == 1
###    assert traj.n_residues == 882
###
###    traj_2 = file_mmtf.load_mmtf(str(mmtf_file_paths[1]))
...
###    positions, topology, unit_lengths, unit_angles = file_mmtf.MMTFTrajectoryFile._read(tests_systems["1SUX"]["1sux.mmtf"], True)
...
###
###    positions, topology, unit_lengths, unit_angles = file_mmtf.MMTFTrajectoryFile._read(tests_systems["5ZMZ"]["5zmz.mmtf"], True)
