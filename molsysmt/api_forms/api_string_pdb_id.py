...
def to_file_fasta(item, molecular_system, atom_indices='all', structure_indices='all', output_filename=None):
    ...
    tmp_item = string_pdb_id_to_file_fasta(item, digest=False)
    group_indices = get(molecular_system, element='atom', indices=atom_indices, group_index=True, digest=False)
    group_indices = np.unique(group_indices)
    return extract_file_fasta(tmp_item, group_indices=group_indices, output_filename=output_filename, copy_if_all=False, digest=False)
...
def to_mmtf_MMTFDecoder(item, molecular_system, atom_indices='all', structure_indices='all'):
    ...
    return string_pdb_id_to_mmtf_MMTFDecoder(item, atom_indices=atom_indices, structure_indices=structure_indices, digest=False)
...
def to_molsysmt_MolSys(item, molecular_system, atom_indices='all', structure_indices='all',
    ...
    return string_pdb_id_to_molsysmt_MolSys(item, atom_indices=atom_indices,
            structure_indices=structure_indices, bioassembly_name=bioassembly_name, digest=False)


def to_molsysmt_Topology(item, molecular_system, atom_indices='all', structure_indices='all'):
    from molsysmt.form.string_pdb_id import to_molsysmt_Topology as string_pdb_id_to_molsysmt_Topology

    return string_pdb_id_to_molsysmt_Topology(item, atom_indices=atom_indices, structure_indices=structure_indices, digest=False)


def to_molsysmt_Structures(item, molecular_system, atom_indices='all', structure_indices='all'):
    from molsysmt.form.string_pdb_id import to_molsysmt_Structures as string_pdb_id_to_molsysmt_Structures

    return string_pdb_id_to_molsysmt_Structures(item, atom_indices=atom_indices, structure_indices=structure_indices, digest=False)
...
def to_mdtraj_Trajectory(item, molecular_system, atom_indices='all', structure_indices='all'):
    ...
    return string_pdb_id_to_mdtraj_Trajectory(item, atom_indices=atom_indices, structure_indices=structure_indices, digest=False)


def to_mdtraj_Topology(item, molecular_system, atom_indices='all', structure_indices='all'):
    from molsysmt.form.string_pdb_id import to_mdtraj_Topology as string_pdb_id_to_mdtraj_Topology

    return string_pdb_id_to_mdtraj_Topology(item, atom_indices=atom_indices, structure_indices=structure_indices, digest=False)
...
def to_parmed_Structure(item, molecular_system, atom_indices='all', structure_indices='all'):
    ...
    return string_pdb_id_to_parmed_Structure(item, atom_indices=atom_indices, structure_indices=structure_indices, digest=False)
...
def to_pdbfixer_PDBFixer(item, molecular_system, atom_indices='all', structure_indices='all'):
    ...
    return string_pdb_id_to_pdbfixer_PDBFixer(item, atom_indices=atom_indices, structure_indices=structure_indices, digest=False)
...
def to_openmm_Modeller(item, molecular_system, atom_indices='all', structure_indices='all'):
    ...
    return string_pdb_id_to_openmm_Modeller(item, atom_indices=atom_indices, structure_indices=structure_indices, digest=False)


def to_openmm_Topology(item, molecular_system, atom_indices='all', structure_indices='all'):
    from molsysmt.form.string_pdb_id import to_openmm_Topology as string_pdb_id_to_openmm_Topology

    return string_pdb_id_to_openmm_Topology(item, atom_indices=atom_indices, structure_indices=structure_indices, digest=False)
...
def to_openmm_PDBFile(item, molecular_system, atom_indices='all', structure_indices='all'):
    ...
    return string_pdb_id_to_openmm_PDBFile(item, atom_indices=atom_indices, structure_indices=structure_indices, digest=False)
...
def to_mdanalysis_Universe(item, molecular_system, atom_indices='all', structure_indices='all'):
    ...
    return string_pdb_id_to_mdanalysis_Universe(item, atom_indices=atom_indices, structure_indices=structure_indices, digest=False)
...
def to_pytraj_Trajectory(item, molecular_system, atom_indices='all', structure_indices='all'):
    ...
    return string_pdb_id_to_pytraj_Trajectory(item, atom_indices=atom_indices, structure_indices=structure_indices, digest=False)
...
def to_string_pdb_text(item, molecular_system, atom_indices='all', structure_indices='all'):
    ...
    return string_pdb_id_to_string_pdb_text(item, atom_indices=atom_indices, structure_indices=structure_indices, digest=False)
...
def to_nglview_NGLWidget(item, molecular_system, atom_indices='all', structure_indices='all'):
    ...
    return string_pdb_id_to_nglview_NGLWidget(item, atom_indices=atom_indices, structure_indices=structure_indices, digest=False)
