...
class WrongItemError(ValueError):
...
class WrongItemFormError(ValueError):

    def __init__(self, item, form, message=None, caller=None):

        if not caller:
            caller = caller_name(skip=3)

        full_message = f"Error in {caller} due to the form of the `item` argument."
