...
def rotate_and_translate_single_structure(coordinates, center_rotation, rotation_matrix, translation, atom_indices):
    ...
    for ii in atom_indices:
        ...
        aux_vect=transpmatmul(rotation_matrix[a_r,:,:],aux_vect)
        coordinates[ii,:]=aux_vect+translation[a_t,:]
...
output=None
@nb.njit(make_numba_signature(arguments,output), cache=True)
def rotate_and_translate(coordinates, center_rotation, rotation_matrix, translation, atom_indices, structure_indices):
    ...
    for ii in structure_indices:
        ...
        for jj in atom_indices:
            ...
            aux_vect=transpmatmul(rotation_matrix[s_r,a_r,:,:],aux_vect)
            coordinates[ii,jj,:]=aux_vect+translation[s_t,a_t,:]
            if not single_atom_translation:
                a_t+=1
            if not single_atom_center:
                a_c+=1
