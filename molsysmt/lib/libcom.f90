...
CONTAINS
  ...
  FUNCTION GEOMETRICAL_CENTER(coors, groups_indices, groups_atoms_indices, groups_starts,& 
                          n_frames, n_atoms, n_groups, n_groups_atoms) RESULT(center)

    IMPLICIT NONE

    INTEGER,INTENT(IN)::n_frames, n_atoms, n_groups, n_groups_atoms
    ...
    DOUBLE PRECISION,DIMENSION(n_frames, n_groups, 3)::center
 
    weights(:)=1.0d0

    center=CENTER_OF_MASS(coors, groups_indices, groups_atoms_indices, groups_starts, weights,& 
                          &n_frames, n_atoms, n_groups, n_groups_atoms)
