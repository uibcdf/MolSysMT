...
@digest()
def add_arrows(positions, arrows, color=[0.6, 0.6, 0.6], radius='0.2 angstroms'):
    ...
    positions: Quantity (value:numpy.ndarray, dimensionality:['L']=1)
    ...
    color: list, tuple, default=[0.6, 0.6, 0.6]
       RGB color code of the arrows.
    ...
    >>> positions = msm.get(molecular_system, element='atom', selection='atom_name=="CA"', coordinates=True)
    >>> arrows = puw.quantity(np.ones([positions.shape[0],3]), 'angstroms')
    >>> view = msm.view(molecular_system)
    >>> msm.thirds.add_arrows(view, positions, arrows)
    >>> view
    ...
    positions = puw.get_value(positions, to_unit='angstroms')
    positions = puw.get_value(positions, to_unit='angstroms')
    radius = puw.get_value(radius, to_unit='angstroms')

    n_arrows=positions.shape[0]
    end_arrows = positions+arrows
    
    for ii in range(n_arrows):
    
        kwargs = {'position1':positions[ii].tolist(),
