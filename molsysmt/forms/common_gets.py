...
def _aux_getter_big_attribute_from_small(item, method_name, indices):
...
def _aux_getter_small_attribute_from_big(item, method_name, indices):
    ...
    aaa = get(item, target=from_target, indices=indices, **dict_aaa)
    bbb = np.unique(np.concatenate(aaa))
    ccc = get(item, target=to_target, indices=bbb, **dict_ccc)
    aux_dict = dict(zip(bbb, ccc))
    vv = np.vectorize(aux_dict.__getitem__)
    output = np.array([vv(ii) for ii in aaa],dtype=object)
    del(aux_dict)
    return output
