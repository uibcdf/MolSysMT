import numpy as _np
from .multitool import convert as _convert, extract as _extract, get_info as _get_info
...
def radius_of_gyration(item=None, selection=None, frame=None, engine='molmodmt'):

    if engine=='molmodmt':

        tmp_item, atom_indices, frame_indices = _digest_one_system(item, selection, frame,
                                                                      form='molmodmt.Trajectory')
        tmp_coors, tmp_n_frames, tmp_n_atoms = _digest_coordinates(tmp_item, atom_indices, frame_indices, form='molmodmt.Trajectory')
        masses = _get_info(tmp_item, selection=atom_indices, masses=True)
        ...
        masses = _get_info(tmp_item1, masses=True)
