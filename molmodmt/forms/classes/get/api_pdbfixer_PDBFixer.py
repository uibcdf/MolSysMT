...
def get(item, element='atom', indices=None, ids=None, **kwargs):

    #from puede ser 'atom', 'residue', 'chain', 'trajectory'

    result=[]
    args = set([ii for ii in kwargs if kwargs[ii]])
    ...
    if element=='atom':
        ...
        for option in args:
            if option=='n_atoms':
                ...
                residue_indices = get(item, indices=indices, residue_index=True)
                ...
                residue_names = get(item, indices=indices, residue_name=True)
                n_aminoacids=0
