def from_mmtf_MMTFDecoder(item, atom_indices='all', frame_indices='all', bioassembly_index=0,
    ...
    from molmodmt.native.io.dataframe import from_mmtf_MMTFDecoder as dataframe_from_mmtf_MMTFDecoder
    from molmodmt.native.io.composition import from_molmodmt_DataFrame as composition_from_molmodmt_DataFrame
    from numpy import reshape, sum

    dataframe = dataframe_from_mmtf_MMTFDecoder(item, atom_indices=atom_indices,
            frame_indices=frame_indices, bioassembly_index=bioassembly_index,
            bioassembly_name=bioassembly_name)

    tmp_item = composition_from_molmodmt_DataFrame(dataframe)

    from  molmodmt.native.elements import BioAssembly_Transformation
