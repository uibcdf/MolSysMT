...
CONTAINS
  ...
  FUNCTION LEAST_RMSD(coors_ref,list_ref,coors,list,n_atoms_ref,n_list_ref,n_frames,n_atoms,n_list) RESULT(rmsd_val)
    ...
    DOUBLE PRECISION,DIMENSION(n_list_ref,3)::g
    
    INTEGER::i,j,N,ll
    DOUBLE PRECISION,DIMENSION(n_list_ref,3)::x,y
  ...
  SUBROUTINE LEAST_RMSD_FIT(coors_ref,list_ref,coors,list,n_atoms_ref,n_list_ref,n_frames,n_atoms,n_list)
    ...
    DO i=1,N
       sw=w(i)
       x(i,:)=sw*dble(coors_ref(list_ref(i)+1,:))
    END DO
    ...
    DO ll=1,n_frames
      ...
      DO i=1,N
         coors(ll,i,:)=matmul(transpose(U(:,:)),coors(ll,i,:)-center_2)+center_ref
      END DO
   END DO 
  
END SUBROUTINE LEAST_RMSD_FIT
