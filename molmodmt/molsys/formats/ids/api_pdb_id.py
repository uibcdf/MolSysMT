...
def to_pdb(form_id,output_file=None):
    from molsysmt.utils.miscellanea import download_pdb as _download_pdb
    return _download_pdb(form_id.split(':')[-1],output_file)

def to_native_Native(form_id):
    from molsysmt.formats.files.api_pdb import to_native_Native as _pdb_to_native_Native
...
def to_mdtraj_Topology(form_id):
    from molsysmt.formats.files.api_pdb import to_mdtraj_Topology as _pdb_to_mdtraj_Topology
...
def to_mdtraj(form_id):
    from molsysmt.utils.miscellanea import download_pdb as _download_pdb
    from molsysmt.formats.files.api_pdb import to_mdtraj as _pdb_to_mdtraj
...
def to_parmed_Structure(form_id):
    from molsysmt.formats.files.api_pdb import to_parmed_Structure as _pdb_to_parmed_Structure
    _tmp_file=to_pdb(form_id)
...
def to_yank_Topography(form_id):
    from molsysmt.formats.files.api_pdb import to_yank_Topography as _pdb_to_yank_Topography
    _tmp_file=to_pdb(form_id)
